"""
Train Siamese distinguisher on a ciphertext-pair dataset.
...
The first half of each sample is treated as C, the second half as C'.
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="Train Siamese distinguisher")
    ...
    p.add_argument("--embed_dim", type=int,   default=64,
                   help="Embedding size per branch")
...
def train(args):
    ...
    # branch_dim = half of input (C and C' are equal length)
    assert X.shape[1] % 2 == 0, \
        f"Input dim {X.shape[1]} must be even (concat representation required)"
    branch_dim = X.shape[1] // 2
    ...
    # ── Model ──────────────────────────────────────────────────────────────────
    model = SiameseNet(branch_dim=branch_dim, embed_dim=args.embed_dim)

    print(f"SiameseNet  branch_dim={branch_dim}  embed_dim={args.embed_dim}")
